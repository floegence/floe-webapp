...
Small control wells use a one-pixel inner shade and an unblurred one-pixel light edge. Radio dots are flat solid glyphs; do not apply container relief to a 6–10px mark. Switch thumbs and filled progress portions stay shadow-free by default; their fixed recessed tracks carry depth. Repainting even one-pixel shadows on frequently moving or resizing faces adds measurable dense-update cost. Explicit public shadow overrides remain authoritative. Disabled Checkbox and Switch content is dimmed once by its label, not again by its face. Review these controls at both 1× and 2× pixel density, not only in enlarged previews.

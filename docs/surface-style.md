...
- Input focus changes only existing border color. In soft neumorphic mode the existing bottom edge carries the full focus color while the other edges retain their quiet color. This avoids a strong rectangular frame without diluting the semantic focus color. Error and high-contrast fields retain complete colored boundaries. Dimensions, position, padding, border width, and decorative shadow are identical before and after focus, including intermediate animation frames. Compound fields use `data-floe-input-surface`; editable descendants stay frameless.

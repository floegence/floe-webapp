...
The package dependency is pinned to the published `@floegence/flowersec-core@5.2.0` package. Protocol uses only Flowersec's current root, browser, and proxy entrypoints. It does not expose an arbitrary Controller factory or a second control-plane fetch/decode facade; all acquisition imports come from `@floegence/floe-webapp-boot`.

## HTTP Direct opt-in

`ConnectConfig` also accepts `{ source, httpDirect: { origin, ...options } }`,
where the source is an `HTTPDirectArtifactSourceV1`. Protocol delegates to
Flowersec's `createHTTPDirectConnectionControllerV1`; it does not reinterpret TLS
or private-loopback credentials. `controller`, `privateLoopback`, and `httpDirect`
are mutually exclusive connection profiles for one client, not server access
permissions. Changing a profile requires `replaceConnection()`, just like changing
a source. Multiple providers own independent sessions and disconnect independently.

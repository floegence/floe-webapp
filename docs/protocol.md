...
The package dependency is pinned to the published `@floegence/flowersec-core@5.1.0` package. Protocol uses only Flowersec's current root, browser, and proxy entrypoints. It does not expose an arbitrary Controller factory or a second control-plane fetch/decode facade; all acquisition imports come from `@floegence/floe-webapp-boot`.

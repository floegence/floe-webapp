...
`dockItemActivationMode` controls plain clicks on built-in Dock components. The default, `solo-filter`, preserves the existing filter behavior. `focus-cycle` keeps every component visible: the first activation re-centers the current same-type selection or focuses the first spatial candidate; repeated activations advance top-to-bottom and then left-to-right, wrapping after the final candidate. Creation time and ID provide stable tie-breakers. A manual selection, another Dock item, or an added, removed, or reordered candidate starts a new cycle; waiting and canvas panning do not. An empty type is created at the current viewport center. Navigation animates without changing zoom and transfers focus into the target; `onDockItemClick` still runs first and skips this default when it returns `true`.

The `focus-cycle` mode keeps instance state in its accessible label and tooltip without covering Dock icons with count badges. Empty types include `+`, and the active candidate uses `current/total`. Pointer clicks and native Enter/Space activation share the same path. Dragging a component still creates it at the drop point and never also fires the click action. Host Dock items, Dock actions, mode switching, and context menus keep their existing semantics.

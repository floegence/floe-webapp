...
- Digit-to-copy is a shared browse-mode affordance: after the user interacts with the notes canvas, pressing a note number copies that note body, flashes the copied state, and shows a toast. Exact prefix matches such as `1` vs `12` stay pending briefly so multi-digit note numbers remain usable.
- Digit shortcuts are intentionally disabled while the user is typing, while editor/manual-paste/context-menu/trash surfaces are open, and before the user has entered canvas browse mode. Title-only notes keep click-to-edit behavior, while digit-copy shows an informational toast instead of unexpectedly opening the editor.

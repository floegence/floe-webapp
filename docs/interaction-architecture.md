...
所有 modal、模态 drawer 与 command palette 都要共用统一遮罩契约：
...
### 2.5 non-modal anchored companion 语义

`BottomBarCompanion` 是从 Bottom Bar 布局锚点向上展开的非模态 disclosure。它不是 modal drawer、popover、`FloatingWindow` 或 `SurfaceFloatingLayer`，也不能通过组合这些组件来模拟。
...
- Escape 使用 bubble listener，让 consumer-owned Portal 先消费；未被消费且焦点属于 companion 时才收起。
- 该 surface 无 backdrop、body lock、focus trap，也不阻断背景交互，因此不使用 `useOverlayMask()`。产品负责关闭后的领域焦点策略。
- 动画只发生在离散的 disclosure 状态切换，不与 pointer drag/resize 热路径并发；reduced motion 直接提交目标 frame。

结论：

- 先按交互语义分类，再决定是否使用 overlay mask。视觉上“浮在内容上方”并不自动等于 modal overlay。
- 产品可把一棵稳定业务树 Portal 到 companion content host 或其它显式 host，但共享组件不克隆、不重挂载、也不拥有该业务树。

### 2.6 可访问性也是共享交互契约的一部分
...
3. modal overlay 必须优先使用 `useOverlayMask()`；non-modal anchored companion 必须遵守 2.5 的独立 disclosure 契约
...
6. projected widget shell 与 rich body 必须显式分层：shell 负责基于共享 projected rect 做 `translate3d(...) scale(...)` 投影、size、z-index、drag、resize、selection、focus；body 保持按 `widget.id` 稳定挂载，禁止因为 viewport 或 sibling 几何变化而 remount。
7. rich widget body 必须通过 `surfaceMetrics` accessor 感知自己的 projected rect 与 ready 状态，而不是自行猜测 mount host 是否稳定，也禁止把每帧新建的 metrics 对象直接作为 value prop 广播给全部 body。
...
12. terminal / editor / projected preview 这类带 proxy textarea、隐藏 focus sink、虚拟输入桥接节点的 widget，必须在自己的 widget-owned 激活边界上显式声明 `data-floe-workbench-widget-activation-surface="true"`；共享 shell 看到该边界后，必须继续走 widget activation contract，而不是把内部 helper 节点误判成“原生 typing target 已经赢得第一次点击”。
...
3. `clearSelection()` / 空白画布 `pointerdown` 不仅要清空 `selectedWidgetId`，还必须把 runtime input owner 交还给 canvas，并把 DOM 焦点收回 shared surface root；这样鼠标回到未选中 widget 上方时，wheel 仍然响应 canvas zoom，而不是残留在旧 widget 的局部滚动语义里。
4. `focusWidget(...)` 保留 center-only 语义，用于 shell activation、keyboard navigation、ensure-widget 等不应改变 zoom level 的入口。
5. `fitWidget(...)` 表示“将 widget 完整 fit 到当前 viewport”，必须通过共享 helper 计算目标 scale 与中心点，禁止下游重复手写公式。
6. `overviewWidget(...)` 表示“将 widget 置中并回到最小 canvas scale”，用于从局部工作状态回到全局空间感。
7. context menu root 必须声明显式菜单边界；outside-dismiss 使用 `pointerdown capture`，并统一覆盖 Escape、scroll、resize。
8. backdrop 只能参与视觉遮罩和右键 retarget，不应再依赖 backdrop `click` 作为唯一关闭机制，否则 transformed / portal 场景下容易抢走 menu item click。
9. 任何 `focus / fit / overview` 这类程序化 viewport 动画，如果遇到用户 `wheel / pan` 直接操控，必须立即取消；不要再让 `live viewport` 与导航动画互相覆盖。
...
`CodeEditor` 这类共享 surface 不允许再假设所有 standalone service 都应无条件加载，也不允许把 Monaco 底层 service 拆装知识继续泄漏给下游业务组件。

规则如下：

1. Monaco standalone runtime 必须以语义化 profile 维度缓存，而不是“一次性全量导入后全局复用”。
2. preview / readonly surface 应该优先通过 `runtimeOptions.profile` 选择共享 profile（例如 `preview_basic`）；当前安全口径是在 shared runtime helper 里显式注册 Monaco standalone 运行时实际会触发的 service 模块，再用 editor options 做只读/轻交互裁剪，而不是继续把“空 runtime + 手动关 service”留给下游业务组件自己承担。
3. richer editor surface 可以继续使用默认 profile（`editor_full`），并且应由共享 runtime helper 直接引导到 Monaco 官方完整入口，而不是在业务代码里手工拼装一组内部 service 模块。
4. 历史 `runtimeOptions.standaloneFeatures` 只允许作为兼容输入，并且必须在共享 runtime helper 中归一化到安全 profile；下游产品代码不应再新增这类调用。
5. runtime loader 失败后必须允许同一 profile 重试，不能把失败状态永久缓存。
6. 共享测试必须覆盖：默认 runtime、preview runtime、legacy compatibility 归一化，以及不同 profile 的独立缓存行为。
...
- [x] BottomBarCompanion 提供独立的 non-modal anchored disclosure，不复用 FloatingWindow / SurfaceFloatingLayer / overlay mask
- [x] BottomBarCompanion 保留单一 shell/content host，并原子提交 ready mount + anchor frame
...
1. 判断该交互是“点击类”、“热几何类”、“modal overlay 类”还是“non-modal anchored companion 类”。
...
5. 如果组件是 modal overlay，先接 `useOverlayMask()`，再补视觉样式；如果是 non-modal anchored companion，使用独立 disclosure contract，禁止为了复用视觉外壳而套用 window、point layer 或 modal mask。
6. 写完后补守卫测试，至少证明：
   - 共享契约被复用

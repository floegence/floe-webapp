...
2. overlay 打开时优先根据最近一次 `pointerdown capture` / `focusin` 交互快照，回溯最近的 overlay host。
3. 找到局部 host 时，overlay 必须以局部 surface 模式挂载：
   - `Portal mount={host}`
   - modal root / menu panel 使用局部 `absolute` 布局，而不是继续假设自己总在全局 viewport
   - menu / dropdown / submenu 必须先按 viewport rect 计算目标位置，再把坐标投影到当前 host 的本地坐标系

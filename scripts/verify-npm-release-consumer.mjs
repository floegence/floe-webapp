...
execFileSync(
  ...
  [
    ...
    [
      ...
      `resource.set(['Retained'])`,
      `const cancelled = resource.refresh(() => new Promise(() => {})).catch(error => error.name)`,
      `cache.cancelRefreshes('consumer')`,
      `if (await cancelled !== 'AbortError' || resource.snapshot().data[0] !== 'Retained') throw new Error('scope cancellation discarded its snapshot')`,
      `cache.dispose()`,

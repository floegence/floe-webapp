...
function assertInitTemplates() {
  ...
  for (const template of ['minimal', 'full']) {
    ...
    assert(
      templatePackage.dependencies?.['@floegence/floe-webapp-core'] === '^0.44.2',
      `Init template ${template} must target @floegence/floe-webapp-core ^0.44.2`
    );
  }
...
function main() {
  ...
  assert(
    [corePkg.version, bootPkg.version, protocolPkg.version, initPkg.version].every(
      (version) => version === '0.44.2'
    ),
    'Published Floe packages must all use version 0.44.2'
  );

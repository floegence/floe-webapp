...
function assertInitTemplates() {
  ...
  for (const template of ['minimal', 'full']) {
    ...
    assert(
      templatePackage.dependencies?.['@floegence/floe-webapp-core'] === '^0.46.2',
      `Init template ${template} must target @floegence/floe-webapp-core ^0.46.2`
    );
  }
...
function main() {
  ...
  assert(
    ...
    'Published Floe packages must all use version 0.46.2'
  );

...
function assertInitTemplates() {
  ...
  for (const template of ['minimal', 'full']) {
    ...
    assert(
      templatePackage.dependencies?.['@floegence/floe-webapp-core'] === '^0.45.0',
      `Init template ${template} must target @floegence/floe-webapp-core ^0.45.0`
    );
  }
...
function main() {
  ...
  assert(
    ...
    'Published Floe packages must all use version 0.45.0'
  );

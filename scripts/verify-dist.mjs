...
function assertFileEquals(pathA, pathB, hint = '') {
...
function assertInitTemplates() {
  ...
  for (const template of ['minimal', 'full']) {
    ...
    assert(
      templatePackage.dependencies?.['@floegence/floe-webapp-core'] === '^0.41.1',
      `Init template ${template} must target @floegence/floe-webapp-core ^0.41.1`
    );
  }
...
function assertSkillContract(corePkg) {
  const skillPath = 'skills/floe-webapp/SKILL.md';
  const playbookPath = 'skills/floe-webapp/references/playbooks.md';
  const skill = readFileSync(resolve(process.cwd(), skillPath), 'utf-8');
  const playbooks = readFileSync(resolve(process.cwd(), playbookPath), 'utf-8');
...
function main() {
  ...
  assert(
    [corePkg.version, bootPkg.version, protocolPkg.version, initPkg.version].every((version) => version === '0.41.1'),
    'Published Floe packages must all use version 0.41.1'
  );

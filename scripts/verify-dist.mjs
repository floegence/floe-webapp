...
function assertInitTemplates() {
  ...
  for (const template of ['minimal', 'full']) {
    ...
    assert(
      templatePackage.dependencies?.['@floegence/floe-webapp-core'] === '^0.42.3',
      `Init template ${template} must target @floegence/floe-webapp-core ^0.42.3`
    );
  }
...
function main() {
  ...
  assert(
    [corePkg.version, bootPkg.version, protocolPkg.version, initPkg.version].every(
      (version) => version === '0.42.3'
    ),
    'Published Floe packages must all use version 0.42.3'
  );

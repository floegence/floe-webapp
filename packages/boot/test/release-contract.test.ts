...
describe('release dependency and runtime contract', () => {
  ...
  it('keeps all published package versions aligned', () => {
    ...
    expect(corePkg.version).toBe('0.74.2');
  ...
  it('uses the published flowersec-core release without local dependency shortcuts', () => {
    ...
    expect(goModule).toMatch(
      /^require github\.com\/floegence\/flowersec\/flowersec-go\/v5 v5\.3\.1$/mu
    );
    ...
    expect(goChecksums).toContain('flowersec-go/v5 v5.3.1');
  ...
  it('validates the frozen dependency graph before running the local quality gate', () => {
    ...
    expect(makefile).toMatch(
      /^check: install flowersec-smoke-peer lint typecheck test build verify packed-consumer window-status workbench-header persistent-scrollbar chat-media activity-navigation input-history pdf asset-recovery resource-cache reload-placeholder remote-input mobile-viewport overlay-viewport file-loading$/mu
    );

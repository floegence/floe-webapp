...
describe('release dependency and runtime contract', () => {
  ...
  it('keeps all published package versions aligned', () => {
    ...
    expect(corePkg.version).toBe('0.37.0');
  ...
  it('uses the published flowersec-core release without local dependency shortcuts', () => {
    ...
    expect(bootPkg.dependencies?.['@floegence/flowersec-core']).toBe('^0.20.0');
    expect(protocolPkg.dependencies?.['@floegence/flowersec-core']).toBe('^0.20.0');
    expect(lockfile).toContain("'@floegence/flowersec-core@0.20.0':");

...
describe('release dependency and runtime contract', () => {
  ...
  it('keeps all published package versions aligned', () => {
    ...
    expect(corePkg.version).toBe('0.36.76');
  ...
  it('uses the published flowersec-core release without local dependency shortcuts', () => {
    ...
    expect(bootPkg.dependencies?.['@floegence/flowersec-core']).toBe('^0.19.11');
    expect(protocolPkg.dependencies?.['@floegence/flowersec-core']).toBe('^0.19.11');
    expect(lockfile).toContain("'@floegence/flowersec-core@0.19.11':");

...
describe('release dependency and runtime contract', () => {
  ...
  it('keeps all published package versions aligned', () => {
    ...
    expect(corePkg.version).toBe('0.64.0');
  ...
  it('validates the frozen dependency graph before running the local quality gate', () => {
    ...
    expect(makefile).toMatch(
      /^check: install flowersec-smoke-peer lint typecheck test build verify packed-consumer workbench-header persistent-scrollbar chat-media activity-navigation input-history$/mu
    );

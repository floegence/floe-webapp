...
export type SpendCommitRequest = SpendBindingView &
  Readonly<{
    attemptId: string;
    receipt: string;
  }>;
...
type IsolatedOneShotState = {
  readonly pending: PendingAcquisition<ArtifactLease>;
  state: 'ready' | 'connecting' | 'consumed';
};

const acquisitionSources = new WeakMap<AcquisitionSource, AcquisitionSourceState>();
...
type ValidatedAcquisition<Artifact> = Readonly<{
  artifact: Artifact;
  scope: ProxyRuntimeScope;
...
export function registerAcquisitionSource(source: AcquisitionSource): void {
...
export function synchronizeAcquisitionSourceSnapshot(
  ...
  if (state.connected !== undefined && state.connectedSession === snapshot.currentSession)
...
export function clearAcquisitionSource(source: AcquisitionSource): void {
  const state = acquisitionSources.get(source);
...
export function connectedAcquisitionDetails(
...
export async function materializeIsolatedOneShot(
  ...
  const oversized =
    ...
    byteLength(`${HANDOFF_PREFIX}${rawHandoff}`) > MAX_ISOLATED_FRAGMENT_BYTES;
  let cleared = false;
  try {
  ...
  if (consumedHandoffDigests.has(handoffDigest))
    throw new AcquisitionError('isolated_handoff_consumed');
  ...
  if (pending === undefined || pending.lease !== lease)
    throw new AcquisitionError('isolated_materialization_failed');
...
export async function connectIsolatedOneShot(
  ...
  if (state === undefined || state.state !== 'ready')
    throw new AcquisitionError('isolated_acquisition_consumed');
...
async function validateAcquisitionEnvelope<Artifact>(
...
): Promise<ValidatedAcquisition<Artifact>> {
  ...
  if (
    envelope.v !== 1 ||
    typeof envelope.connect_artifact !== 'string' ||
    envelope.connect_artifact.length === 0 ||
    typeof envelope.critical_scope_projection_json !== 'string' ||
    envelope.critical_scope_projection_json.length === 0
  ) {
  ...
  await verifyDigest(
    projectionBytes,
    spendScope.projection_digest_b64u,
  ...
  if (
    projection.scope !== 'proxy.runtime' ||
    projection.scope_version !== 2 ||
  ...
  try {
    artifact = parse(envelope.connect_artifact);
  } catch {
  ...
  if (
    customBindingIdentity !== undefined &&
    (typeof customBindingIdentity !== 'string' ||
      customBindingIdentity.trim() === '' ||
      customBindingIdentity !== customBindingIdentity.trim())
  ) {
  ...
  return Object.freeze({
    ...
    binding,
    bindingIdentity:
      customBindingIdentity ?? `${binding.artifactDigestB64u}.${binding.projectionDigestB64u}`,
    receipt: spendScope.receipt,
  });
}

function materializeLease<Artifact, Lease extends AcquisitionLease>(
  validated: ValidatedAcquisition<Artifact>,
  createLease: (artifact: Artifact, commitSpend: (signal?: AbortSignal) => Promise<void>) => Lease,
  commitSpend: CommitSpend,
  registered: (pending: PendingAcquisition<Lease>) => void,
  failed: (pending: PendingAcquisition<Lease>) => void
): Lease {
  ...
  let pending!: PendingAcquisition<Lease>;
  const lease = createLease(artifact, async (signal) => {
...
function validateSpendScope(
  ...
  if (
    scope.v !== 1 ||
    typeof scope.receipt !== 'string' ||
    !validReceipt(scope.receipt) ||
    typeof scope.artifact_digest_b64u !== 'string' ||
    !validDigest(scope.artifact_digest_b64u) ||
    typeof scope.projection_digest_b64u !== 'string' ||
    !validDigest(scope.projection_digest_b64u) ||
    scope.consumer !== expectedConsumer ||
    typeof scope.expires_at !== 'string' ||
    !validRfc3339(scope.expires_at) ||
...
function validateRuntimeBootPayload(
  value: unknown,
  ...
  if (
    payload.v !== 6 ||
    payload.env_public_id !== expected.envPublicId ||
    payload.floe_app !== expected.floeApp ||
    payload.code_space_id !== expected.codeSpaceId ||
    payload.app_path !== expected.appPath ||
    payload.launcher_kind !== expected.launcherKind ||
    payload.launcher_id !== expected.launcherId ||
    payload.launcher_origin !== expected.launcherOrigin ||
    payload.runtime_origin !== expected.runtimeOrigin ||
...
function validateIsolatedCrossBinding(
  ...
  if (
    ...
    binding.launcherOrigin !== payload.launcher_origin ||
    binding.runtimeOrigin !== payload.runtime_origin ||
    binding.appOrigin !== payload.app_origin ||
    payload.runtime_origin !== expected.runtimeOrigin ||
    payload.app_origin !== expected.appOrigin
  ) {
...
async function verifyDigest(bytes: Uint8Array, expected: string, code: string): Promise<void> {
  ...
  for (let index = 0; index < actual.length; index += 1)
    mismatch |= actual[index]! ^ expectedBytes[index]!;
...
function randomBase64Url32(): string {
  if (globalThis.crypto?.getRandomValues === undefined)
    throw new AcquisitionError('acquisition_entropy_unavailable');

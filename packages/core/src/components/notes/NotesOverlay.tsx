...
import { Check, Copy, FileText, Pencil, Plus, Trash, X } from '../../icons';
...
function clampMenuPosition(clientX: number, clientY: number, itemCount: number): NotesPoint {
  const height = estimateNotesContextMenuHeight(itemCount);
  return {
    x: Math.max(12, Math.min(clientX, window.innerWidth - NOTES_CONTEXT_MENU_WIDTH_PX - 16)),
...
export function NotesOverlay(props: NotesOverlayProps) {
  ...
  const loading = createMemo(() => props.controller.loading?.() ?? false);
  const streaming = createMemo<NotesConnectionState>(() => props.controller.connectionState?.() ?? 'idle');
  const activeTopicID = createMemo(() => props.controller.activeTopicID());
  const viewport = createMemo(() => props.controller.viewport());
  ...
  function commitViewport(next: NotesViewport | ((current: NotesViewport) => NotesViewport)): void {
    props.controller.setViewport(typeof next === 'function' ? next(viewport()) : next);
  }
  ...
  async function saveNoteEditor(): Promise<void> {
    ...
    if (!note) return;
    try {
      await props.controller.updateNote(note.note_id, {
        body: editorBody(),
  ...
  async function createTopic(): Promise<void> {
    ...
    if (!name) return;
    try {
      const topic = await props.controller.createTopic({ name });
      props.controller.setActiveTopicID(topic.topic_id);
  ...
  async function commitTopicRename(topic: NotesTopic): Promise<void> {
    ...
    try {
      await props.controller.updateTopic(topic.topic_id, { name });
    } catch (error) {
  ...
  async function deleteTopicByID(topicID: string): Promise<void> {
    try {
      const deleted = await props.controller.deleteTopic(topicID);
  ...
  async function createNoteAt(worldX: number, worldY: number, body: string, options?: { openEditor?: boolean }): Promise<void> {
    ...
    try {
      const item = await props.controller.createNote({
  ...
  async function copyNote(note: NotesItem): Promise<void> {
    void Promise.resolve(props.controller.bringNoteToFront(note.note_id)).catch(() => undefined);
  ...
  async function deleteNote(note: NotesItem): Promise<void> {
    ...
    try {
      await props.controller.deleteNote(note.note_id);
    } catch (error) {
      notify.error('Delete failed', error instanceof Error ? error.message : String(error));
    }
  }
  ...
  async function restoreTrashItem(item: NotesTrashItem): Promise<void> {
    try {
      const restored = await props.controller.restoreNote(item.note_id);
      props.controller.setActiveTopicID(restored.topic_id);
  ...
  async function clearTrashGroup(group: NotesTrashGroup): Promise<void> {
    try {
      await props.controller.clearTrashTopic(group.topic_id);
    } catch (error) {
  ...
  function beginGesture(event: PointerEvent, noteID: string | null): void {
    ...
    if (gesture.pointerType !== 'mouse') {
      ...
      gesture.longPressTimer = window.setTimeout(() => {
        ...
        untrack(() => openContextMenuAt(gesture!.startClientX, gesture!.startClientY, gesture!.noteID));
      }, LONG_PRESS_MS);
    }
  }
  ...
  createEffect(() => {
    ...
    if (!active && topics().length > 0) {
      props.controller.setActiveTopicID(topics()[0]!.topic_id);
    }
  ...
  const contextMenuItems = createMemo<NotesContextMenuItem[]>(() => {
    ...
    if (!state) return [];
    const menuItems: NotesContextMenuItem[] = [];
  ...
  function wheelZoom(event: WheelEvent): void {
    ...
    commitViewport((current) => zoomViewportAtPoint(current, current.scale * scaleDelta, anchorX, anchorY));
  }
  ...
  function onViewportTouchMove(event: TouchEvent): void {
    ...
    commitViewport((current) => zoomViewportAtPoint(current, nextScale, anchorX, anchorY));
  }
  ...
  function focusTopicCenter(worldX: number, worldY: number): void {
    ...
    commitViewport((current) => centerViewportOnWorldPoint(current, worldX, worldY, size.width || 1, size.height || 1));
  }

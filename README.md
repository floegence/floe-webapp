...
Filename rules match exact names, patterns and the longest registered suffix before an extension hint for extensionless entries. The icon catalog identifies naming conventions; archive extraction and file preview support remain separate capabilities. The approved standalone study is retained only as a private regression fixture at `packages/core/test/fixtures/approved-file-icons.html`; it is not shipped in the demo. Tests verify all 156 approved families against frozen compact/detail geometry, palettes and matching rules; existing generic code aliases remain supported as an additional family.

...
`CodeEditor` now exposes profile-first Monaco runtime bootstrapping through `runtimeOptions.profile`. Use `preview_basic` for lightweight preview/read-only surfaces that still need syntax highlighting, and keep `editor_full` for richer editor experiences. `editor_full` deliberately boots through Monaco's official `editor.main` entry so downstream apps do not need to hand-maintain Monaco's internal standalone service graph. The older `runtimeOptions.standaloneFeatures` input remains as a compatibility shim and is normalized onto those safe profiles instead of letting downstream code toggle Monaco's low-level service graph directly.
